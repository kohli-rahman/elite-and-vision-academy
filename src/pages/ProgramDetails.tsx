...
const ProgramDetails = () => {
  ...
  // Extended program details including exam information
  const programs = {
    // IIT-JEE Programs
    'iit-jee-foundation': {
                ...
                <div className="glass-card p-6 rounded-xl">
                  <div className="mb-4">
                    <h3 className="text-lg font-semibold mb-2 flex items-center">
                      <FileText className="h-5 w-5 text-primary mr-2" /> {program.examInfo.name}
                    </h3>
                    <p className="text-muted-foreground">{program.examInfo.description}</p>
                  </div>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div>
                      <h4 className="font-medium mb-2 text-primary">Examination Structure</h4>
                      <p className="text-sm text-muted-foreground">{program.examInfo.structure}</p>
                    </div>
                    <div>
                      <h4 className="font-medium mb-2 text-primary">Importance</h4>
                      <p className="text-sm text-muted-foreground">{program.examInfo.importance}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="section-container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(programs)
              .filter(([key]) => key !== id && key.includes(id?.split('-')[0] || ''))
              .slice(0, 3)
